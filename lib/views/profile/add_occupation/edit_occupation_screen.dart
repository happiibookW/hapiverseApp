...
class EditOccupationsScreen extends StatefulWidget {
  // const EditOccupationsScreen({Key? key}) : super(key: key);
  final Data? data;

  const EditOccupationsScreen({super.key, required this.data});

  @override
  State<EditOccupationsScreen> createState() => _EditOccupationsState();
}

class _EditOccupationsState extends State<EditOccupationsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    workspaceName = widget.data?.workSpaceName ?? '';
    location = widget.data?.location ?? '';
    description = widget.data?.description! ?? '';
    currentlyWorking = widget.data?.currentWorking == 1 ? true : false;
    startDate = DateTime.parse(widget.data?.startDate ?? DateTime.now().toString());
    endDate = DateTime.parse(widget.data?.endDate ?? DateTime.now().toString());


    print("Hello===> "+title);
    print("Hello===> "+workspaceName);
    print("Hello===> "+location);
    print("Hello===> "+description);
    print("Hello===> "+currentlyWorking.toString());
                      ...
                      SecendoryButton(
                          ...
                          onPressed: () {
                            if (restate.occupation_id.toString() == 'null' || restate.occupation_id.isNullOrEmpty) {
                              ...
                              bloc.updateOccupation(authBloc.userID!, authBloc.accesToken!, b, context,widget.data!.id.toString());
                            }
                          }),
                    ],
                  ),
